...
const fetchedValue = ref<number | null>(null)
const series = computed(() => [fetchedValue.value ?? props.value])

type WorkItemsStatusItem = { status?: string; name?: string; count?: number; value?: number }
const tryParsePercent = (workItemsStatus: WorkItemsStatusItem[] | undefined | null) => {
  if (!Array.isArray(workItemsStatus) || workItemsStatus.length === 0) return null
  let completed = 0
  let total = 0
  for (const item of workItemsStatus) {
    const status = String(item.status ?? item.name ?? '').toLowerCase()
    const count = Number(item.count ?? item.value ?? 0)
    if (Number.isFinite(count)) {
      total += count
      if (status === 'completed' || status === 'done') completed += count
    }
  }
...
const fetchSummary = async () => {
  try {
    const res = await insightsService.getSummary({ limit: 10 })
    const percent = tryParsePercent(res.workItemsStatus)
    if (typeof percent === 'number' && Number.isFinite(percent)) {
      fetchedValue.value = Math.max(0, Math.min(100, percent))
    }
  } catch {}
}
