    ...
    >
      ...
      <div class="flex items-end justify-between mt-5">
        <div>
          ...
          <h4 class="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">{{ customersLabel }}</h4>
        </div>
    ...
    >
      ...
      <div class="flex items-end justify-between mt-5">
        <div>
          ...
          <h4 class="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">{{ ordersLabel }}</h4>
        </div>
...
const customersLabel = computed(() => formatNumber(customersCount.value))
const ordersLabel = computed(() => formatNumber(ordersCount.value))

const fetchCounts = async () => {
  try {
    const [contactsRes, workItemsRes] = await Promise.all([
      crmService.listContacts({ limit: 1 }),
      crmService.listWorkItems({ limit: 1 }),
    ])
    customersCount.value = contactsRes.count ?? contactsRes.data.length
    ordersCount.value = workItemsRes.count ?? workItemsRes.data.length
  } catch {}
}
