...
const mapAlertToNotification = (alert: Alert): NotificationItem => {
  const userName = alert?.contact?.name || alert?.company?.name || 'Alerta'
  const action = alert?.message || 'Nueva alerta'
  const project = alert?.company?.name || ''
  const type = String(alert?.severity || 'info')
  const status: 'online' | 'offline' = type === 'high' || type === 'critical' ? 'online' : 'offline'
  return {
